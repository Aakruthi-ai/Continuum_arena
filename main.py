...
async def run_game():
    ...
    # --- Setup Tutorial Simulation Properties ---
    tut_pulse = 0.0
    ...
    while running:
                ...
                if current_tab == "CREATOR":
                    ...
                    for idx in range(4):
                        if pygame.Rect(center_view_x + 30, header_h + 75 + (idx * 85), lobby_box_w, 68).collidepoint(m_pos):
                            ...
                            # Shuffle Missions on map launch
                            active_missions = generator_shuffle_missions()

                            # Re-generate procedurally structured obstacles map layout
        ...
        if current_tab == "CREATOR":
            ...
            # --- LIVE CANVAS BOUNDARY PARAMETERS ---
            ...
            # --- IN-TUTORIAL LIVE ACTIVE VECTOR SIMULATION MECHANIC LOOP ---
            if tut_state_timer == 0:  # Setup or Reset targets positions
            ...
            # Processing Tutorial Animation States
            if tut_state_timer == 1: # Step 1: Steering towards the Time Orb
                target = tut_orb_pos
                ...
                if dir_v.length() > 3:
                    ...
                    tut_float_timer = 50
                    tut_state_timer = 2
            elif tut_state_timer == 2: # Step 2: Steering towards the Mystery Box
                target = tut_box_pos
                dir_v = target - tut_char_pos
                if dir_v.length() > 3:
                    tut_char_pos += dir_v.normalize() * 1.5
                    ...
                    tut_float_timer = 50
                    tut_state_timer = 3
            elif tut_state_timer == 3: # Step 3: Idle reset delay sequence
                tut_state_timer += 1
            elif tut_state_timer > 3:
                tut_state_timer += 1
                if tut_state_timer > 120:  # Loop back
                    tut_state_timer = 0

            # Render Animated Grid Vectors
            ...
            # Draw Character Core Simulation Model
