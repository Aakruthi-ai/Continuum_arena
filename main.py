...
async def run_game():
    ...
    def get_fonts():
        scale = max(0.85, min(1.5, HEIGHT / 640.0))
        return {
            "main": pygame.font.SysFont("OCR A Extended", int(15 * scale)),
            "small": pygame.font.SysFont("Verdana", int(12 * scale), bold=False),
    ...
    # --- Projectile Classes ---
    class Bullet:
        def __init__(self, x, y, angle, color, owner_name):
    ...
    class ObstacleBlock:
        def __init__(self, start_x):
            ...
            self.rect = pygame.Rect(
                random.randint(int(start_x + (playable_w * 0.3)), int(start_x + (playable_w * 0.7))),
    ...
    class MysteryBoxDrop:
        ...
        def draw(self, surface):
            ...
            # Render a neat neon question mark inside the drop box
    ...
    class Fighter:
        def __init__(self, x_ratio, y_ratio, color, is_ai=False, name="VEX-9"):
            ...
            self.has_mystery_control = False  # Track if picked up box
        ...
        def handle_input(self, keys, stats_speed, bullets, blocks, min_x, touch_target_pos=None):
            ...
            if touch_target_pos is not None:
                ...
                if delta_move.length() > 5:
                    ...
                    self.ratio.x = (new_x - min_x) / (WIDTH - min_x)
                    self.ratio.y = new_y / HEIGHT
            else:
                dx, dy = 0, 0
                if keys[pygame.K_w]: dy -= 1; self.is_moving = True
                if keys[pygame.K_s]: dy += 1; self.is_moving = True
                if keys[pygame.K_a]: dx -= 1; self.is_moving = True
                if keys[pygame.K_d]: dx += 1; self.is_moving = True
                ...
                if self.is_moving:
                    ...
                    if move_vec.length() > 0:
                        ...
                        self.ratio.x += move_vec.x / (WIDTH - min_x)
                        self.facing_angle = math.atan2(move_vec.y, move_vec.x)
            ...
            if self.shoot_cooldown > 0: 
                ...
                # Fire Condition router based on Mystery Box collection status
                should_fire = False
                if self.has_mystery_control:
                    # Auto firing rapid mode engaged!
                    should_fire = (random.random() < 0.06)
                else:
                    # Strictly manual firing mode
                    should_fire = (keys[pygame.K_SPACE] or touch_target_pos is not None)

                if should_fire:
                    p = self.get_actual_pos(min_x)
                    bullets.append(Bullet(p.x, p.y - 18, self.facing_angle, self.color, self.name))
                    self.shoot_cooldown = 10 if self.has_mystery_control else 14
                    play_sound(550, 0.06)
        ...
        def draw(self, surface, min_x):
            ...
            w_len = h_scale * 2.4
            pygame.draw.line(surface, GOLD, (p.x, p.y - h_scale + bounce), (p.x + math.cos(self.facing_angle)*w_len, p.y - h_scale + bounce + math.sin(self.facing_angle)*w_len), 3)
    ...
    history, bullets, orbs, blocks, active_boxes = [], [], [], [], []
    orb_spawn_timer = 0
    box_spawn_timer = 0
    boxes_spawned_count = 0  # Counter to track strict cap of 2 spawns
    ...
    while running:
        ...
        screen.blit(fonts["title"].render("CONTINUUM-ARENA // MANUAL CORE SYSTEM", True, WHITE), (15, 18))
        ...
        advice_text = [
            ...
            "🏆 AND Hostile Bots eliminated",
            "📦 Boxes enable AUTOMATIC RAPID fire!",
            "⚠️ Purging bots without log entries = LOSE!"
        ]
                ...
                if current_tab == "CREATOR":
                    ...
                    for idx in range(4):
                        if pygame.Rect(center_view_x + 30, header_h + 75 + (idx * 85), lobby_box_w, 68).collidepoint(m_pos):
                            ...
                            enemies.clear(); bullets.clear(); history.clear(); orbs.clear(); blocks.clear(); active_boxes.clear()
        ...
        if current_tab == "CREATOR":
            ...
            screen.blit(fonts["title"].render("INTELLIGENCE ARCHIVE: GUN MECHANICS", True, GOLD), (center_view_x + 30, header_h + 25))
            card_w = int((WIDTH - center_view_x - 60) / 2)
            card_h = int((HEIGHT - header_h - 100) / 2)
            card_h = max(130, card_h)

            x1, y1 = center_view_x + 25, header_h + 70
            b1 = pygame.Rect(x1, y1, card_w, card_h)
            pygame.draw.rect(screen, PANEL_BG, b1, border_radius=6); pygame.draw.rect(screen, NEON_CYAN, b1, 1, border_radius=6)
            screen.blit(fonts["main"].render("1. 100% MANUAL TRIGGER CONTROL", True, WHITE), (b1.x + 15, b1.y + 12))
            render_wrapped_text(screen, "Automatic loop shooting logic has been wiped completely. Your ship will never fire a tracking projectile on its own cycle.", fonts["small"], GRAY, pygame.Rect(b1.x + 15, b1.y + 38, card_w - 30, 80))

            x2, y2 = x1 + card_w + 15, y1
            b2 = pygame.Rect(x2, y2, card_w, card_h)
            pygame.draw.rect(screen, PANEL_BG, b2, border_radius=6); pygame.draw.rect(screen, NEON_PURPLE, b2, 1, border_radius=6)
            screen.blit(fonts["main"].render("2. HOW TO FIRE THE WEAPON", True, WHITE), (b2.x + 15, b2.y + 12))
            render_wrapped_text(screen, "Press and hold the SPACEBAR on your keyboard, or press/tap coordinates within the field frame to deploy linear energetic lasers.", fonts["small"], GRAY, pygame.Rect(b2.x + 15, b2.y + 38, card_w - 30, 80))

            x3, y3 = x1, y1 + card_h + 15
            b3 = pygame.Rect(x3, y3, card_w, card_h)
            pygame.draw.rect(screen, PANEL_BG, b3, border_radius=6); pygame.draw.rect(screen, NEON_AMBER, b3, 1, border_radius=6)
            screen.blit(fonts["main"].render("3. FACING ORIENTATION MECHANIC", True, WHITE), (b3.x + 15, b3.y + 12))
            render_wrapped_text(screen, "Projectiles follow the golden line indicator attached to your chassis center. Move your player to adjust your targeting vector angles.", fonts["small"], GRAY, pygame.Rect(b3.x + 15, b3.y + 38, card_w - 30, 80))

            x4, y4 = x2, y3
            b4 = pygame.Rect(x4, y4, card_w, card_h)
            pygame.draw.rect(screen, PANEL_BG, b4, border_radius=6); pygame.draw.rect(screen, NEON_GREEN, b4, 1, border_radius=6)
            screen.blit(fonts["main"].render("4. MYSTERY BOX AUTO FIRE UNLOCK", True, WHITE), (b4.x + 15, b4.y + 12))
            render_wrapped_text(screen, "Grab a glowing purple Mystery Box to instantly toggle automatic tracking fire on. Max 2 boxes spawn per match simulation.", fonts["small"], GRAY, pygame.Rect(b4.x + 15, b4.y + 38, card_w - 30, 80))

        elif current_tab == "BATTLE_FIELD":
            p1.max_hp = 1000 + (stats["DEFENSE"] * 50)
            p1_calc_atk = (15 + (stats["STRENGTH"] * 2))
            ...
            if is_rewinding:
                ...
                if history:
                    ...
                    bullets = [Bullet(b[0], b[1], b[2], b[3], b[4]) for b in snap[5]]
                ...
                if time_scale > 0 and winning_character == "":
                    # 1. Manage Time Orb Spawns
                    orb_spawn_timer += 1
                    if orb_spawn_timer > 150: orbs.append(TimeOrb(center_view_x)); orb_spawn_timer = 0

                    # 2. Manage Mystery Box Spawns (Strict Max Cap of 2)
                    if boxes_spawned_count < 2 and len(active_boxes) == 0:
                        box_spawn_timer += 1
                        # Drop a new mystery box roughly every 6-7 seconds if none exist
                        if box_spawn_timer > 380:
                    ...
                    # 3. Process picking up Time Orbs
                    for o in orbs:
                        o.update()
                    ...
                    # 4. Process picking up Mystery Boxes
                    for box in active_boxes[:]:
                        if p1.is_alive and p1_box.colliderect(box.rect):
                            p1.has_mystery_control = True  # Activates auto fire loop
                            active_boxes.remove(box)
                            play_sound(1100, 0.25)

                    # 5. Process laser collisions and borders
                    for b in bullets[:]:
                    ...
                    if p1.is_alive:
                        ...
                        b_sn = [(b.pos.x, b.pos.y, b.angle, b.color, b.owner_name) for b in bullets]
                        o_sn = [(o.pos.x, o.pos.y) for o in orbs]
                        m_sn = {k: {"current": v["current"], "done": v["done"]} for k, v in active_missions.items()}
                        
                        history.append((pygame.Vector2(p1.ratio), p1.hp, p1.is_alive, match_timer, e_sn, b_sn, o_sn, m_sn, p1.has_mystery_control, boxes_spawned_count))
            ...
            f_mode_text = "FIRE MODE: AUTO RAPID" if p1.has_mystery_control else "FIRE MODE: 100% MANUAL"
            f_mode_col = NEON_PURPLE if p1.has_mystery_control else NEON_GREEN
            screen.blit(fonts["small"].render(f_mode_text, True, f_mode_col), (center_view_x + 210, header_h + 25))
