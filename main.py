...
async def run_game():
    ...
    NEON_RED = (255, 50, 50)
    WHITE = (240, 240, 240)
    GRAY = (140, 145, 160)  # Made text slightly brighter for readability
    GOLD = (255, 215, 0)
    ...
    # --- Optimized Font Scaling Engine (Boosted sizing) ---
    def get_fonts():
        scale = max(0.85, min(1.5, HEIGHT / 640.0))  # Bumped minimum scale factor up
        return {
            "main": pygame.font.SysFont("OCR A Extended", int(15 * scale)),
            "small": pygame.font.SysFont("Verdana", int(12 * scale), bold=False), # Slightly cleaner reading weight
    ...
    class ObstacleBlock:
        def __init__(self, start_x):
            ...
            min_allowed_x = int(start_x + (playable_w * 0.40))
            max_allowed_x = int(start_x + (playable_w * 0.70))
            
            self.rect = pygame.Rect(
                random.randint(min_allowed_x, max_allowed_x),
    ...
    while running:
        ...
        # Dynamic Layout Panel Dimensions
        ...
        # --- Active Dashboard View Render Pipeline ---
        if current_tab == "CREATOR":
        ...
        # --- RESTORED COMFORTABLE TOP-TO-BOTTOM ARCHIVE GRID ---
            ...
            # Dynamic adaptive grid setup matching original roomy card distribution
            card_w = int((WIDTH - center_view_x - 60) / 2)
            card_h = int((HEIGHT - header_h - 100) / 2)
            card_h = max(130, card_h)

            # --- CARD 1: LOCOMOTION VECTOR ---
            ...
            screen.blit(fonts["main"].render("1. LOCOMOTION VECTOR", True, WHITE), (b1.x + 15, b1.y + 12))
            # Text has full width allocation now!
            render_wrapped_text(screen, "Maneuver your unit across space using standard WASD layout parameters.", fonts["small"], GRAY, pygame.Rect(b1.x + 15, b1.y + 38, card_w - 30, 45))
            
            # Visual Diagram rendered centered directly underneath description text
            v_cx, v_cy = b1.centerx, b1.y + int(card_h * 0.74)
            for kw, kx, ky, active in [("W", 0, -14, False), ("A", -24, 10, True), ("S", 0, 10, False), ("D", 24, 10, False)]:
                k_box = pygame.Rect(v_cx + kx - 10, v_cy + ky - 10, 20, 20)
                pygame.draw.rect(screen, (0, 80, 95) if active else (20, 20, 45), k_box, border_radius=3)
                pygame.draw.rect(screen, NEON_CYAN if active else GRAY, k_box, 1, border_radius=3)
                screen.blit(fonts["small"].render(kw, True, WHITE if active else GRAY), (k_box.x + 5, k_box.y + 3))

            # --- CARD 2: LASER INTERCEPT ---
            ...
            screen.blit(fonts["main"].render("2. LASER INTERCEPT", True, WHITE), (b2.x + 15, b2.y + 12))
            render_wrapped_text(screen, "Press SPACEBAR to unleash active plasma rounds toward incoming configurations.", fonts["small"], GRAY, pygame.Rect(b2.x + 15, b2.y + 38, card_w - 30, 45))
            
            # Visual Diagram rendered centered directly underneath description text
            v_cx, v_cy = b2.centerx - 25, b2.y + int(card_h * 0.74)
            pygame.draw.circle(screen, NEON_PURPLE, (v_cx, v_cy), 8) 
            pygame.draw.line(screen, WHITE, (v_cx + 8, v_cy), (v_cx + 45, v_cy), 2) 
            pygame.draw.rect(screen, NEON_RED, (v_cx + 45, v_cy - 6, 12, 12), border_radius=2) 

            # --- CARD 3: CHRONO REVERSAL ---
            ...
            screen.blit(fonts["main"].render("3. CHRONO REVERSAL", True, WHITE), (b3.x + 15, b3.y + 12))
            render_wrapped_text(screen, "Hold key [R] at any crunch frame to rewind health, position, and vectors.", fonts["small"], GRAY, pygame.Rect(b3.x + 15, b3.y + 38, card_w - 30, 45))
            
            # Visual Diagram rendered centered directly underneath description text
            v_cx, v_cy = b3.centerx, b3.y + int(card_h * 0.74)
            pygame.draw.arc(screen, NEON_AMBER, (v_cx - 20, v_cy - 20, 40, 40), 0.5, 5.5, 2) 
            pygame.draw.circle(screen, NEON_PURPLE, (v_cx + 18, v_cy - 8), 5) 
            pygame.draw.polygon(screen, NEON_AMBER, [(v_cx - 15, v_cy + 10), (v_cx - 25, v_cy + 15), (v_cx - 15, v_cy + 22)])

            # --- CARD 4: TIME CORE MATRIX ---
            x4, y4 = x2, y3
            ...
            screen.blit(fonts["main"].render("4. TIME CORE MATRIX", True, WHITE), (b4.x + 15, b4.y + 12))
            render_wrapped_text(screen, "Intercept spawned golden spheres directly to gain critical timeline extensions.", fonts["small"], GRAY, pygame.Rect(b4.x + 15, b4.y + 38, card_w - 30, 45))
            
            # Visual Diagram rendered centered directly underneath description text
            v_cx, v_cy = b4.centerx - 15, b4.y + int(card_h * 0.74)
            pygame.draw.circle(screen, NEON_PURPLE, (v_cx, v_cy), 8) 
            pygame.draw.circle(screen, GOLD, (v_cx + 30, v_cy), 6) 
            pygame.draw.circle(screen, NEON_GREEN, (v_cx + 30, v_cy), 10, 1) 
